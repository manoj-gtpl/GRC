...
import { fetchOrganizations as getOrganizations } from "../api/organizations";
...
export default function DeveloperDashboard() {
  ...
  const loadOrganizations = async () => {
    try {
      setLoading(true);
      const orgs = await getOrganizations(); // ✅ imported function
    ...
    try {
      if (selectedOrg) {
        ...
        const response = await fetch(`${API_BASE_URL}/api/organization-admin/register/`, {
          ...
          body: JSON.stringify({
            organization: selectedOrg.organization_id, // existing org ID
            admin_name: orgData.admin.admin_name,
            admin_email: orgData.admin.admin_email,
            admin_phone: orgData.admin.admin_phone,
            admin_password: orgData.admin.admin_password,
            role_id: orgData.admin.role,
          }),
        });

        if (!response.ok) throw new Error("Failed to register organization admin");
  ...
  const filteredOrganizations = organizations.filter((org) => {
    const matchesSearch =
      ...
      org.industry_type?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const deleteOrgName =
    deleteOrgId && organizations.find((o) => o.organization_id === deleteOrgId)?.name;

  const hasExistingAdmin = organizations.some((org) => org.admin && org.organization_id !== selectedOrg?.organization_id);
